...
console.log(document.querySelectorAll('.addWpsd'));
document.querySelectorAll('.addWpsd').forEach((ele) =>{
  if (ele != null){
    ele.addEventListener('input' ,()=>{
      if (ele.value>0){
        product_map.set(ele.getAttribute("name").substring(7), ele.value);
      } else {
        ele.value=0;
        product_map.set(ele.getAttribute("name").substring(7), ele.value);
      }
      ...
      str=str.trim();
      console.log(str);
      document.querySelector(".wpsdString").value=str;
    });
  }
});
