...
public class EmployeeLibrary {
	public static ArrayList<String> viewEmployeeList(Pair<ArrayList<EmployeeObject>,Integer> datas, Quartet<EmployeeObject, Short, Byte, EMPLOYEE_SORT_TYPE> infors){
		...
		Iterator<EmployeeObject> eItem = datas.getValue0().iterator(); 
		...
		while (eItem.hasNext()) {
			EmployeeObject item = eItem.next();
			tmp.append("<tr>");		
			...
			tmp.append("<td>"+item.getUser_office_phone()+"</td>");
			tmp.append("<td>"+item.getUser_email()+"</td>");
			tmp.append("<td><span class=\"badge bg-primary\">Active</span></td>");
			tmp.append("<td>"+item.getEmployee_workplace_id()+"</td>");
			tmp.append("<td>"+item.getEmployee_workplace_start_time()+"</td>");
			tmp.append("<td>");
