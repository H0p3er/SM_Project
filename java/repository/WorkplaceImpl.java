...
public class WorkplaceImpl extends BasicImpl implements Workplace {
	...
	@Override
	public synchronized boolean addWorkplace(ArrayList<WorkplaceObject> wItems, ArrayList<WpsdObject> wsdItem, EmployeeObject currentUser) {
		...
		try {
			if (workplaceId.next()) {
				...
				if (result && wsdItem != null && wsdItem.size()>0) {								
					...
					sql.append("wpsd_workplace_id, wpsd_product_id, ");
	...
	@Override
	public boolean editWorkplace(ArrayList<WorkplaceObject> wItems, ArrayList<WpsdObject> pItem, WORKPLACE_EDIT_TYPE et, EmployeeObject currentUser) {
		...
		switch (et) {
			case GENERAL:
				sql.append("workplace_name=?, workplace_type=?, workplace_address=?, ");
				sql.append("workplace_website_link=?, workplace_map_link=?, ");
				sql.append("workplace_last_modified_id=?, workplace_last_modified_date=?, ");
				sql.append("workplace_images=?, workplace_type=? ");
			break;
		...
		try {
			...
			switch(et) {
				case GENERAL:
					...
					pre.setString(4, wItem.getWorkplace_website_link());		
					pre.setString(5, wItem.getWorkplace_map_link());
					pre.setInt(6, wItem.getWorkplace_last_modified_id());
					pre.setString(7, wItem.getWorkplace_last_modified_date());	
					pre.setString(8, wItem.getWorkplace_images());
					pre.setByte(9, wItem.getWorkplace_type());	
	...
	@Override
	public boolean delWorkplace(ArrayList<WorkplaceObject> wItems, EmployeeObject currentUser) {
		// TODO Auto-generated method stub
		WorkplaceObject wItem = wItems.get(0);
	...
	public synchronized ArrayList<ResultSet> getWorkplaces(
		...
		if (isDetail) {
			...
			sql.append("SELECT bill_import_target_workplace_id, tblbill.bill_created_date, SUM(bill_detail_product_price*bill_detail_product_quantity) as import_value FROM tblbill ");
			sql.append("INNER JOIN tblbilldetail ON tblbill.bill_id=tblbilldetail.bill_detail_id ");
			sql.append("WHERE bill_import_target_workplace_id = "+similar.getWorkplace_id()+" AND bill_type=0 ");
			sql.append("GROUP BY bill_created_date, bill_import_target_workplace_id ");
			sql.append("ORDER BY STR_TO_DATE(bill_created_date, '%e/%c/%Y') ASC ");		
			sql.append(";");
			
			sql.append("SELECT bill_export_current_workplace_id, tblbill.bill_created_date, SUM(bill_detail_product_price*bill_detail_product_quantity) as export_value FROM tblbill ");
			sql.append("INNER JOIN tblbilldetail ON tblbill.bill_id=tblbilldetail.bill_detail_id ");
			sql.append("WHERE bill_export_current_workplace_id = "+similar.getWorkplace_id()+" AND bill_type=1 ");
			sql.append("GROUP BY bill_created_date, bill_export_current_workplace_id ");
			sql.append("ORDER BY STR_TO_DATE(bill_created_date, '%e/%c/%Y') ASC ");		
			sql.append(";");
			
			sql.append("SELECT tblproduct.*, bill_created_date, wpsd_workplace_id, AVG(bill_detail_product_price) AS AvgExport, SUM(bill_detail_product_quantity) as TotalExport FROM tblwpsd ");
			...
			sql.append("WHERE bill_type=1 AND wpsd_deleted=0 AND wpsd_workplace_id="+similar.getWorkplace_id()+" ");
			sql.append("GROUP BY product_id, bill_created_date, wpsd_workplace_id ");
			...
			sql.append("SELECT * FROM tblworkplace ");
			sql.append("LEFT JOIN tbluser ON tblworkplace.workplace_creator_id=tbluser.user_id ");
			...
			switch (type) {	
				...
				case ADDRESS:
					...
					break;
				case CREATED:
					sql.append(" ORDER BY ORDER BY STR_TO_DATE(workplace_created_date, '%e/%c/%Y') DESC ");
					break;
				case MODIFIED:
			...
			sql.append("SELECT tblproduct.*, wpsd_workplace_id, bill_created_date, MAX(bill_detail_product_price) AS AvgExport, SUM(bill_detail_product_quantity) as TotalExport FROM tblwpsd ");
			sql.append("LEFT JOIN tblproduct ON tblwpsd.wpsd_product_id=tblproduct.product_id ");
			sql.append("LEFT JOIN tblbilldetail ON tblbilldetail.bill_detail_product_id=tblwpsd.wpsd_product_id ");
			sql.append("LEFT JOIN tblbill ON tblbill.bill_id=tblbilldetail.bill_detail_id ");	
		...
		sql.append("SELECT COUNT(workplace_id) AS total FROM tblworkplace LEFT JOIN tbluser ");
		sql.append("ON tblworkplace.workplace_creator_id=tbluser.user_id ");
		sql.append(this.createCondition(similar,currentUser));
		sql.append(";");					
...
//		System.out.print(sql.toString());		
	...
	private String SELECTCondition(WorkplaceObject similar, EmployeeObject currentUser) {
	...
	private String createCondition(WorkplaceObject similar, EmployeeObject currentUser) {
		 StringBuilder conds = new StringBuilder();
		 if (currentUser!=null) {
			 ...
			 if (id>0) {				 
				 if (permis<4) {			 
					 conds.append("AND ( (user_parent_id=").append(id).append(") OR (workplace_creator_id=").append(id).append(") OR (workplace_creator_id=").append(id).append(") )") ;
				 } else {
			 ...
			 if (similar!=null) {
				 //Kiểm tra tồn tại
				 if (similar.isWorkplace_deleted()) {
					 conds.append("AND (workplace_deleted=1)");
				 } else {
					 conds.append("AND (workplace_deleted=0)");
				 }
				 
				 //Xử lí từ khóa tìm kiếm
				 String key = similar.getWorkplace_name();
				 if (key!=null && !key.equalsIgnoreCase("")) {
					 conds.append(" AND (");
					 conds.append("(workplace_name LIKE '%"+key+"%') OR ");
					 conds.append("(workplace_address LIKE '%"+key+"%') ");
					 conds.append(") ");
				 }		 
			 }		
		 }
	...
	public static void main(String[] args) {
		...
		EmployeeObject currentUser = new EmployeeObject();
...
//		boolean result = u.addWorkplace(new_WorkplaceList,sdl, currentUser);
//		
//		if (!result) {
//			System.out.print("\n---------------------Khong thanh cong---------------------\n");
//		}
		...
		Byte ,
		WORKPLACE_SORT_TYPE,
		Boolean> infors = new Sextet<>(currentUser,new_Workplace,0,(byte)25, WORKPLACE_SORT_TYPE.NAME,false);
...
//					row += "\tADDRESS: "+rs.getString("workplace_address");
					...
					row += "\tWP: "+rs.getInt("wpsd_workplace_id");
