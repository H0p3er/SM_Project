...
import connection.*;
import constant.EMPLOYEE_EDIT_TYPE;
...
public class EmployeeImpl extends UserImpl implements Employee {
	...
	public ResultSet getEmployee(String username, String userpass) {
		// TODO Auto-generated method stub
		 ...
		 String sqlSelect = "SELECT * FROM tblemployee INNER JOIN tbluser ON tbluser.user_id=tblemployee.employee_id WHERE (tbluser.user_name=?) AND (tbluser.user_pass=md5(?)) AND (tbluser.user_deleted=0); ";
	...
	public ArrayList<ResultSet> getEmployees(EmployeeObject similar, int at, byte uPerPage, EMPLOYEE_SORT_TYPE type) {
		// TODO Auto-generated method stub
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT * FROM tblemployee LEFT JOIN tbluser ON tblemployee.employee_id = tbluser.user_id ");
		sql.append(this.createCondition(similar));
		switch (type) {
		case NAME:
		...
		case ADDRESS:
			...
			break;
		case CREATED:
			sql.append(" ORDER BY user_created_date DESC ");
			break;
		case MODIFIED:
			sql.append(" ORDER BY user_last_modified_date DESC ");
			break;
		default:
			sql.append(" ORDER BY employee_id ASC ");
		}

		sql.append("LIMIT " + at + "," + uPerPage + "; ");
